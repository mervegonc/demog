...
const ArticleForm = () => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (formData.articleId) {
        response = await axios.put(`http://16.16.43.64:3000/api/article/${formData.articleId}`, articleData, config);
        alert('Updated Successfully');
        navigate(`/onearticle/${response.data.id}`);
      } else {
        response = await axios.post('http://16.16.43.64:3000/api/article', articleData, config);
  ...
  const uploadPhotos = async articleId => {
    ...
    try {
      await axios.put(`http://16.16.43.64:3000/api/article/photos/${articleId}`, photoFormData, config);
