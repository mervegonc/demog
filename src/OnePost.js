            ...
            try {
                ...
                if (postData) {
                    ...
                    if (postData.photoId) {
                        ...
                        const photoResponse = await axios.get(`http://localhost:8080/api/post/photos/${postId}`, { responseType: 'blob' });
                     
                        const photoUrl = URL.createObjectURL(photoResponse.data);
                        setPost(prevPost => ({ ...prevPost, photoUrl }));
                        console.log("Post photo URL:", post.photoUrl);
                    ...
                    // Kullanıcının profil fotoğrafı isteğini gönder
                 
                    const userProfilePhotoResponse = await axios.get(`http://localhost:8080/api/user/profile/${userId}`, { responseType: 'blob' });
                    const userProfilePhotoUrl = URL.createObjectURL(userProfilePhotoResponse.data);
                    setUserProfilePhoto(userProfilePhotoUrl);
                } else {
                    // Eğer postData yoksa hata mesajını yazdır
    ...
    /*useEffect(() => {
        const fetchPost = async () => {
            try {
                const response = await axios.get(`http://localhost:8080/api/post/${postId}`);
                const postData = response.data;
                // Kontrol edilecek: postData'nun doğru şekilde doldurulduğu
                if (postData) {
                    const photoResponse = await axios.get(`http://localhost:8080/api/post/photos/${postId}`, { responseType: 'blob' });
                    const photoUrl = URL.createObjectURL(photoResponse.data);
                    setPost({ ...postData, photoUrl });
                    ...
                    setPost(postData)
                    // postData eksik veya hatalıysa, uygun bir işlem yapılabilir
                    console.error('Post data is missing or incorrect.');
                }
            } catch (error) {
                console.error('Error fetching post:', error);
