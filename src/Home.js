...
const Home = () => {
    ...
    useEffect(() => {
        const fetchFollowingUsersContent = async () => {
            try {
                const followingResponse = await axios.get(`http://16.16.43.64:3000/api/user/${authenticatedUserId}/following`);
                const followingUsers = followingResponse.data.following;

                const postsPromises = followingUsers.map(async userId => {
                    const postsResponse = await axios.get(`http://16.16.43.64:3000/api/post/my/${userId}`);
                ...
                const articlesPromises = followingUsers.map(async userId => {
                    const articlesResponse = await axios.get(`http://16.16.43.64:3000/api/article/my/${userId}`);
