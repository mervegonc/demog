...
const Home = () => {
            ...
            try {
                ...
                const postsPromises = followingUsers.map(async userId => {
                    ...
                    const postsWithMedia = await Promise.all(postsResponse.data.map(async post => {
                        ...
                        return { ...post, mediaContent: [...photoUrls, ...videoUrls], userProfilePhoto, currentMediaIndex: 0 }; // currentMediaIndex eklenmeli
                    }));
                ...
                const articlesPromises = followingUsers.map(async userId => {
                    ...
                    const articlesWithPhotos = await Promise.all(articlesResponse.data.map(async article => {
                        ...
                        return { ...article, photoUrls, userProfilePhoto, currentPhotoIndex: 0 }; // currentPhotoIndex eklenmeli
                    }));
    ...
    const renderCurrentMedia = (mediaContent, currentIndex) => {
        ...
        if (!currentMedia || !currentMedia.type) return null; // Check if currentMedia and currentMedia.type are defined
