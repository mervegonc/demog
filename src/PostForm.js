...
const PostForm = () => {
  const [formData, setFormData] = useState({
    ...
    postId: null,
    createdAt: null,
    articleId: null // Yeni eklenen alan
  });
  const [selectedFiles, setSelectedFiles] = useState([]);
  const [articles, setArticles] = useState([]); // Kullanıcının makaleleri
  const [searchTerm, setSearchTerm] = useState(''); // Arama çubuğu terimi
  ...
  useEffect(() => {
    if (location.state) {
      const { title, text, postId, articleId } = location.state;
      setFormData({ title, text, postId, articleId });
    }
  }, [location]);
  ...
  useEffect(() => {
    if (searchTerm.trim()) {
      const fetchArticles = async () => {
        try {
          const response = await axios.get(`http://localhost:8080/api/article/search?keyword=${searchTerm}&userId=${AuthService.getUserId()}`);
  ...
  const handleTextChange = (e) => {
    const { name, value } = e.target;
    const maxLength = 70; // Her satırın maksimum uzunluğu
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setFormData({ title: '', text: '', createdAt: null, articleId: null }); // createdAt ve articleId alanını sıfırla
      setSelectedFiles([]);
    } catch (error) {
        ...
        <div className='article-search'>
          ...
          <ul className='article-results'>
            {articles.map(article => (
              <li key={article.id} onClick={() => handleArticleSelect(article.id)}>
                {article.subject}
              </li>
            ))}
          </ul>
...
const PostForm = () => {
  ...
  useEffect(() => {
    if (location.state) {
      const { title, text, postId, articleId, articleSubject } = location.state;
      setFormData({ title, text, postId, articleId , articleSubject});
    }
  }, [location]);
  ...
  const handleTextChange = (e) => {
    const { name, value } = e.target;
    const maxLength = 80; // Her satırın maksimum uzunluğu
    const lines = value.split('\n');
    const formattedLines = lines.map(line => {
      if (line.length > maxLength) {
        return line.match(new RegExp(`.{1,${maxLength}}`, 'g')).join('\n');
...
const PostForm = () => {
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      setFormData({ title: '', text: '', createdAt: null, articleId: null });
      setSelectedFiles([]);
  ...
  const uploadVideo = async postId => {
    ...
    try {
      await axios.put(`http://localhost:8080/api/post/videos/${postId}`, videoFormData, config);
  ...
  return (
    <div className='post-form-container'>
      ...
      <div className="video-upload">
        ...
        <input type="file" id="videoUpload" onChange={handleVideoChange} accept="video/*" />
      </div>
...
import FileUpload from './FileUpload';
import savePostImage from './styles/images/savepost.png';
import AddArticleLinkIcon from './styles/images/add.png';
...
const PostForm = () => {
  const [formData, setFormData] = useState({
    ...
    articleId: null,
    articleSubject: null,
    connections: '' // connections alanını ekliyoruz
  });
  const [selectedFiles, setSelectedFiles] = useState([]);
  const [selectedVideo, setSelectedVideo] = useState(null);
  const [articles, setArticles] = useState([]);
...
const youtubeInputRef = useRef(null);
  ...
  const handleConnectionsChange = (e) => {
    ...
    setFormData({ ...formData, connections: value });
  };
  ...
  const handleArticleSelect = (articleId) => {
    setFormData({ ...formData, articleId });
    setShowArticleSearch(false); // Kutu kapansın
  };
  const handleSubmit = async e => {
    e.preventDefault();
    if (!formData.title.trim() || !formData.text.trim()) {
  ...
  const handleAddArticleConnection = () => {
    setShowArticleSearch(!showArticleSearch); // Kutu aç/kapa
  };
  ...
  const handleAddYoutubeConnection = () => {
    setShowYoutubeModal(!showYoutubeModal); // Kutu aç/kapa
  };
  
  const handleYoutubeLinkSubmit = () => {
    const youtubeLink = youtubeInputRef.current.value;
    setFormData({ ...formData, connections: youtubeLink });
    setShowYoutubeModal(false); // Kutu kapansın
  };
