    ...
    const LikeForm = ({ postId }) => {
        ...
        useEffect(() => {
            const fetchLikeCount = async () => {
                try {
                    const response = await axios.get(`http://localhost:8080/api/like/count/${postId}`);
        ...
        useEffect(() => {
            const checkUserLiked = async () => {
                try {
                    if (userId) {
                        const token = localStorage.getItem('token');
                        const response = await axios.get(`http://localhost:8080/api/like/${postId}/user/${userId}`, {
                            headers: {
                                Authorization: `Bearer ${token}`
                            }
                        });
        ...
        const handleUnlike = async () => {
            try {
                const token = localStorage.getItem('token'); // LocalStorage'dan tokeni al
                await axios.delete(`http://localhost:8080/api/like/unlike/${userId}/${postId}`, {
                    headers: {
                        Authorization: `Bearer ${token}` // Authorization headeri ile tokeni gönder
                    }
                });
