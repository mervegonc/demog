...
const Connection = ({ userId }) => {
  ...
  useEffect(() => {
    const fetchConnections = async () => {
      try {
        const response = await axios.get(`http://16.16.43.64:3000/api/user/connections/user/${userId}`, {
          headers: {
  ...
  const handleAddConnection = async () => {
    ...
    try {
      const response = await axios.post(`http://16.16.43.64:3000/api/user/connections`, {
        ...newConnection,
        user: { id: userId }
      }, {
        headers: {
  ...
  const handleDeleteConnection = async (connectionId) => {
    try {
      await axios.delete(`http://16.16.43.64:3000/api/user/connections/delete/${connectionId}`, {
        headers: {
          Authorization: `Bearer ${AuthService.getToken()}`
        }
      });
